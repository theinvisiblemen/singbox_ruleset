...
def init():
    # 删除已有文件夹
    dir_path = os.path.join(current_dir, 'ruleset')
...
def translate_rule():
    ...
    target_folder = os.path.join(current_dir, 'ruleset')
    for entry in os.listdir(source_folder):
        ...
        # 检查是否存在对应的 yaml 文件
        yaml_file = os.path.join(source_dir, f'{entry}.yaml')
        yaml_file_classical = os.path.join(source_dir, f'{entry}_Classical.yaml')
        has_yaml = os.path.exists(yaml_file) or os.path.exists(yaml_file_classical)
        ...
        if has_yaml:
            ...
            translate_source_to_target(entry, source_dir, target_dir, entry)
        
        # 如果有子目录，遍历处理子目录
        if has_subdirs:
            if has_yaml:
                logging.info(f'{entry} has both yaml file and subdirectories, processing both...')
            else:
                logging.info(f'{entry} has no direct yaml file, processing subdirectories...')
            
            for sub_entry in os.listdir(source_dir):
                sub_source_dir = os.path.join(source_dir, sub_entry)
                # 只处理子目录
                if os.path.isdir(sub_source_dir):
                    # 保留父目录结构：ruleset/Game/Roblox/
                    sub_target_dir = os.path.join(target_folder, entry, sub_entry)
                    translate_source_to_target(sub_entry, sub_source_dir, sub_target_dir, f'{entry}/{sub_entry}')

    logging.info(f"finish translating clash rules")

def translate_source_to_target(entry, source_dir, target_dir, path_for_readme):
    ...
    with open(readme_file, 'w') as readme:
        readme.write(f'# {entry}\n\n#### 规则链接\n\n**GitHub**\nhttps://raw.githubusercontent.com/theinvisiblemen/sing-box_ruleset/main/ruleset/{path_for_readme}/{entry}.srs\n\n**CDN**\nhttps://cdn.jsdelivr.net/gh/theinvisiblemen/sing-box_ruleset@main/ruleset/{path_for_readme}/{entry}.srs')
...
def translate_extra():
    ...
    for k, v in extra_surge_conf.items():
        ...
        readme_file = os.path.join(target_dir, f'README.md')
        with open(readme_file, 'w') as readme:
            readme.write(f'# {k}\n\n#### 规则链接\n\n**GitHub**\nhttps://raw.githubusercontent.com/theinvisiblemen/sing-box_ruleset/main/ruleset/{k}/{k}.srs\n\n**CDN**\nhttps://cdn.jsdelivr.net/gh/theinvisiblemen/sing-box_ruleset@main/rule/{k}/{k}.srs')
